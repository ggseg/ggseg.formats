...
is a `brain_data_cortical` containing sf polygons for 2D rendering and vertex indices
for 3D:
...
is a `brain_data_subcortical` with sf polygons and individual 3D meshes:
...
`brain_data_tract` with sf polygons and centerlines that generate tube meshes for 3D:
