---
title: "The brain_atlas object"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{The brain_atlas object}
...
Everything in the ggseg ecosystem starts from a single object: the `brain_atlas`.
...
## What is a brain_atlas?

A `brain_atlas` is an S3 object that bundles five pieces of information into one
...
Finally, `$data` is a `brain_atlas_data` object that holds the actual geometry.
...
is a `cortical_data` containing sf polygons for 2D rendering and vertex indices
for 3D:
...
is a `subcortical_data` with sf polygons and individual 3D meshes:
...
`tract_data` with sf polygons and centerlines that generate tube meshes for 3D:
...
`brain_regions()` returns the sorted unique region names:

```{r}
brain_regions(dk)
```

`brain_labels()` returns the unique labels (the identifiers used to join
geometry):

```{r}
head(brain_labels(dk))
```

`brain_views()` returns the available 2D views:
...
`brain_pal()` retrieves the colour palette. You can pass the atlas object
...
`is_brain_atlas()` tests whether an object has the right class:

```{r}
is_brain_atlas(dk)
is_brain_atlas(mtcars)
```

`as_brain_atlas()` coerces lists with the right structure into a proper
`brain_atlas`:

```{r}
atlas_list <- as.list(dk)
recovered <- as_brain_atlas(atlas_list)
is_brain_atlas(recovered)
```

If you have an atlas object from an older version of ggseg that stored sf data
directly in `$data` instead of using the new `brain_atlas_data` wrapper,
