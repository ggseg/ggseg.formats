...
the region metadata. All of them accept a `brain_atlas` as the first argument,
return a new `brain_atlas`, and play nicely with the pipe.
...
"corpus callosum" %in% brain_regions(no_cc)
```
...
"lateral ventricle" %in% brain_regions(ctx)
```
...
```{r}
renamed <- atlas_region_rename(dk, "banks of superior temporal sulcus", "STS banks")
"STS banks" %in% brain_regions(renamed)
```
...
head(brain_regions(upper))
```

## Managing views

`brain_views()` tells you what 2D views an atlas has:
...
network_info <- data.frame(
  region = c("superior frontal", "precuneus", "inferior parietal", "posterior cingulate"),
...
Each function returns a valid `brain_atlas`, so you can inspect intermediate
